...
# THE ATLAS MAY LIVE OUTSIDE THE WORKSPACE (McNugget, 2026-09-19). `<workspace>/agent-atlas`
# is a working clone that seats edit; the deploy keeps its own pinned checkout and passes it
# here as HESTIA_ATLAS_DIR so a machine has a registry without anyone remembering to clone
# one. Unset means "the workspace clone", exactly as before. It is pinned into the wrapper
# for the reason the workspace is: three triggers must not each resolve it for themselves.
ATLAS_PIN="${HESTIA_ATLAS_DIR:-}"
ATLAS_DIR="${ATLAS_PIN:-$WORKSPACE/agent-atlas/talk-to}"
if [ ! -d "$ATLAS_DIR" ]; then
  echo "install: WARNING — no agent-atlas registry at $(sh_pin "$ATLAS_DIR")." >&2
  echo "         Every trigger will report UNKNOWN ('could not look') until there is one." >&2
fi
...
# AT_PIN is '' when no atlas was pinned, and an empty HESTIA_ATLAS_DIR is read by
# inventory.py as "no pin" (tested) -- so this line is safe to pass unconditionally.
exec env HESTIA_WORKSPACE="\${HESTIA_WORKSPACE:-\$WS_PIN}" HESTIA_ATLAS_DIR="\${HESTIA_ATLAS_DIR:-\$AT_PIN}" "\$PY" "\$BIN_PIN.py" "\$@"
WRAP
chmod 0755 "$BIN"
