#!/usr/bin/env python3
"""Hestia witness hook for Cursor — self-contained, stdlib only.

Wired from ~/.cursor/hooks.json as afterShellExecution + afterFileEdit hooks. Reads
...
def run() -> int:
    ...
    # Cursor fires distinct events with its own schema; map shell commands and
    # file edits onto hestia actions. (afterShellExecution: command/output;
    # afterFileEdit: file_path/edits.)
    if isinstance(event.get("command"), str) and event["command"].strip():
        tool_name = "Shell"
        cmd = event["command"].strip()
        target = cmd if len(cmd) <= 240 else cmd[:237] + "..."
        magnitude = 0.8
    elif isinstance(event.get("file_path"), str):
        tool_name = "Edit"
        target = event["file_path"]
        magnitude = 0.6
    else:
        return 0  # an event we don't witness
    ...
    # Cursor's after* hooks are informational (no error/exit signal): record success.
