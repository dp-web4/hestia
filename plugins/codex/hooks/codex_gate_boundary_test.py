#!/usr/bin/env python3
"""Boundary test for the codex gate — the hook EXECUTED, against a stubbed daemon.

Closes the arm the PR #372 blocking note named: `hestia_gate_mechanism_test.py` passes
`plugin_id="codex"` into the shared mechanism but never executes codex's hook, so the Gate-2
...
The load-bearing arms:
  ...
  - an apply_patch whose target is a gate file is refused the same way (codex's write tool);
...
Run: python3 -m pytest <this file> -q   or   python3 codex_gate_boundary_test.py
"""
import json
import os
import shutil
...
def make_workspace(tmp):
    ...
    Gate 2 imports in-process), codex's scope granted over it via a temp identity."""
    ...
    os.makedirs(os.path.join(ws, "hestia", "plugins", "codex", "hooks"))
    os.makedirs(os.path.join(ws, "hestia", "forum", "codex"))
...
def test_gate_file_write_refused_locally():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        srv = Server(stub)
        try:
            target = os.path.join(ws, "hestia", "plugins", "codex", "hooks", "pre_tool_use.py")
...
def test_apply_patch_to_gate_refused_locally():
    """codex's write tool is apply_patch, whose target lives in the patch body — Gate 1c checks
    the EXTRACTED target paths (destination, not content), so an apply_patch aimed at a gate file
    is refused locally the same as a Write."""
    with tempfile.TemporaryDirectory() as tmp:
        ws = make_workspace(tmp)
        stub = StubDaemon()
        srv = Server(stub)
        try:
            target = os.path.join(ws, "hestia", "plugins", "codex", "hooks", "pre_tool_use.py")
            patch = f"*** Begin Patch\n*** Update File: {target}\n@@\n-old\n+new\n*** End Patch\n"
            rc, err = run_hook(ws, _event("apply_patch", {"input": patch}), srv.endpoint)
            check("rc", rc == 2, f"rc={rc} stderr={err}")
            check("stderr-class", "gate-self" in err, err)
            check("claim-made", "hestia_gate_escalation_claim" in stub.names(), stub.names())
        finally:
...
def test_ordinary_write_uses_policy_path():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        srv = Server(stub)
        try:
            target = os.path.join(ws, "hestia", "forum", "codex", "post.md")
            rc, err = run_hook(ws, _event("Write", {"file_path": target, "content": "x"}),
                               srv.endpoint)
            check("rc", rc == 0, f"rc={rc} stderr={err}")
            check("no-claim", "hestia_gate_escalation_claim" not in stub.names(), stub.names())
...
def test_gate_file_bash_write_refused_locally():
    """The Bash write-position arm: a shell write names its DESTINATION in the command text, so a
    raw command scan catches `> <gate>` locally, before the daemon (GPT #372 hold #2 parity). A
    read command that merely NAMES the gate is a documented FP of the same raw scan — a spurious
    refusal, not a hole; the write-target parse + heredoc handling is #370's shared predicate."""
    with tempfile.TemporaryDirectory() as tmp:
        ws = make_workspace(tmp)
        stub = StubDaemon()
        srv = Server(stub)
...
def main():
    ...
    tests = [
        ...
        ("test_apply_patch_to_gate_refused_locally", test_apply_patch_to_gate_refused_locally),
