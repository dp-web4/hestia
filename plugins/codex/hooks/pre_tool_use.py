...
def _role_bridge():
    ...
    try:
        return _load_mechanism().role_bridge(
...
def _tally_scope(allowed: bool):
    ...
    try:
        _load_mechanism().tally_scope(
            allowed, tally_dir=OBSERVE_DIR, tally_path=_TALLY,
...
def _gate_self_call(tool, args, host_session_id=None):
    ...
    try:
        return _load_mechanism().gate_self_call(
            tool, args, plugin_id=HESTIA_PLUGIN_ID, role=_role_bridge(),
            client_name='hestia-codex-gate-self', host_session_id=host_session_id)
    except Exception:
        return None
...
def _witness_gate_self(event_type, marker, tool_name, rule=None):
    ...
    try:
        return _load_mechanism().witness_gate_self(
            event_type, marker, tool_name, rule,
            plugin_id=HESTIA_PLUGIN_ID, role=_role_bridge(),
            gate_path=os.path.abspath(__file__), client_name='hestia-codex-gate-self',
