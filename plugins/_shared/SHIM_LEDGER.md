...
| `_load_projection` | wiring | 1ec946ea | Exports the vault-rendered projection `$HESTIA_HOME/seats/<member>` into the environment before any shared module is resolved. It cannot live in shared authority for the same reason the loader cannot: it is what makes shared authority findable. Decides nothing — it reads one file, compares the launcher's bootstrap locator against the authority's, and reports; the refusal is `main`'s. Byte-identical across seats by intent (#944, #742). |
