...
| `_apply_launch_role` | wiring | e4fcf461 | Hands the projection's permitted launch roles and the live `HESTIA_ROLE` to the shared engine (`hestia_gate_core.launch_role_verdict`) once shared authority is findable, and records its answer (the miswire for `main` to render, and `HESTIA_ROLE_VERIFIED`). Decides nothing: the predicate, the unset-role exemption and the refusal's words are the core's (#1084). No declared set: the core is not loaded and nothing changes. |
