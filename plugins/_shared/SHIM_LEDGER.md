...
| `discover_endpoint` | wiring | df66729f | Endpoint discovery for the installed gate: `HESTIA_ENDPOINT` or the `$HESTIA_HOME/endpoint` file. Named as wiring by the architecture. |
| `_shared_runtime_dir` | wiring | a45c639b | The installed-only loader's path resolution, byte-identical across seats (#742); the one text that cannot be loaded from shared authority because it is what reaches it. |
| `_load_projection` | wiring | faff4486 | Exports the vault-rendered projection `$HESTIA_HOME/seats/<member>` into the environment before any shared module is resolved. It cannot live in shared authority for the same reason the loader cannot: it is what makes shared authority findable. Decides nothing — it reads one file, compares the launcher's bootstrap locator against the authority's, and reports; the refusal is `main`'s. It keeps the projection's `HESTIA_ROLE_PERMITTED` set aside for `_apply_launch_role`, unjudged (#1084). Byte-identical across seats by intent (#944, #742), except those two lines, which other seats adopt when their projections declare a set. |
| `_apply_launch_role` | refusal-channel | d0d072d1 | Hands the projection's permitted launch roles and the live `HESTIA_ROLE` to the shared engine (`hestia_gate_core.launch_role_verdict`, which owns the predicate, the unset-role exemption and the refusal's words, #1084) and records its answer. It owns two narrow things. (1) With no declared set it does not consult the core; that decides nothing, because the core's own answer for an empty set is the same (None, False) (pinned in launch_role_verdict_test). (2) A declared set that cannot be evaluated, core missing or an old core without the function, becomes `gate.core_unavailable`, a message it composes itself because the core is what is missing. That is this class: it runs at import, and a hook that raises exits 1, which Claude Code treats as non-blocking, so the tool would run ungated (codex P1 on a188cde). |
...
| `_record_plane_e` | wiring | 54ca762f | One call into the core's `record_gate_unavailable` with this seat's id attached. |
...
| `main` | LAW-DEBT | b0de91b8 | Now also RECORDS gate 1's refusals (#1028): `_record_core_refusal`, nested here, is a one-call delegation to the shared `witness_decision_unified`, the same recorder kimi's `_record_refusal` row carries as wiring. It runs after stderr and the exit code are already fixed and never raises, so it adds accountability, not verdict logic; `verdict_available` follows the core's own `innate` flag, not a judgement made here. The meter stays under the pin (28.4 vs 29.0). The spine is channel, event shape and identity. The debt is inline: the `HarnessProfile`'s scope markers, two authored scope-deny messages, the `no-shared-authority` refusal prose, and the branch that keeps the Tier-2 matcher reachable (#741, #844). The row flips to wiring when `main` is translate, evaluate, render. Now also refuses an unbacked or miswired seat config before stdin (#944): the projection is the seat's only configuration source, so a seat without one is not configured and does not act. That refusal is law and is per-seat only because each shim owns its own `main`; #934's `decide()` is where it belongs, and this row is the debt until then — the claude-code pin moved 28.8 -> 28.9 to carry it, stated rather than absorbed. Now also DECLARES this seat's review door (#1050): `main`'s policy-snapshot fetch passes `declares_review_door=True`, which puts `escalation-review:v1` in the `gate_capabilities` self-report the daemon's invitation pool now filters on. The assertion is per-seat by necessity rather than by debt — a shared gate library cannot know its caller's effector set, and when the shared mechanism declared it for every caller it declared it AS cbp-being, whose registry holds no corroborate (kimi-code's review, findings/review-13031.md). So this line is a fact only this harness knows, and the one thing it must not do is default to true. Re-justified 2026-09-17 (#1055, review blocker): the comment annotating that declaration quoted a 40,000-entry window count as a chain total — in the codex row, as "40 times — the most of any member", which is true only for a window band and false for the population. All three now name the measurement basis instead of a number; the counts live in findings/review-13031-verdict.md §A, where they can be re-measured, not in a shim comment where they rot. |
...
| `main` | LAW-DEBT | 098fbcaf | Channel, event shape and identity form the spine. Inline: the `_CLOSURE_FLOOR` substring match, `"Write"` asserted for patch targets, read-class gating of the governor, degraded posture selection, an `AgentPolicy` scope literal, and every deny and escalation sentence (#741, #844). Now also DECLARES this seat's review door (#1050): `main`'s policy-snapshot fetch passes `declares_review_door=True`, which puts `escalation-review:v1` in the `gate_capabilities` self-report the daemon's invitation pool now filters on. The assertion is per-seat by necessity rather than by debt — a shared gate library cannot know its caller's effector set, and when the shared mechanism declared it for every caller it declared it AS cbp-being, whose registry holds no corroborate (kimi-code's review, findings/review-13031.md). So this line is a fact only this harness knows, and the one thing it must not do is default to true. Re-justified 2026-09-17 (#1055, review blocker): the comment annotating that declaration quoted a 40,000-entry window count as a chain total — in the codex row, as "40 times — the most of any member", which is true only for a window band and false for the population. All three now name the measurement basis instead of a number; the counts live in findings/review-13031-verdict.md §A, where they can be re-measured, not in a shim comment where they rot. Re-justified 2026-09-18 (#1055, kimi-code's second-seat review, notice 13150): the declaration sentence named `codex-cli` — this seat's witnessed alias — where the chain records its corroborations under `codex`, zero rows against 142 full-chain, so a reader checking the comment against the chain the way the census does would have counted the named id, found nothing, and held a false refutation of the claim the comment exists to support. The sentence now names the id `HESTIA_PLUGIN_ID` asserts, and `hestia_gate_mechanism_test.py`'s door-comment identity test pins every declaring shim's sentence to its own id, so this cannot re-enter as prose a third time. |
...
| `main` | LAW-DEBT | 28df55a7 | Spine is channel and event shape. Inline: read-class gating, the closure authority choice, the read-allowed and write-escalates dispositions with their prose, degraded posture and its read carve-out, a warn-mode `AgentPolicy` literal, and a working-tree `sys.path` insert for the mechanism (#741, #801, #844). Now also DECLARES this seat's review door (#1050): `main`'s policy-snapshot fetch passes `declares_review_door=True`, which puts `escalation-review:v1` in the `gate_capabilities` self-report the daemon's invitation pool now filters on. The assertion is per-seat by necessity rather than by debt — a shared gate library cannot know its caller's effector set, and when the shared mechanism declared it for every caller it declared it AS cbp-being, whose registry holds no corroborate (kimi-code's review, findings/review-13031.md). So this line is a fact only this harness knows, and the one thing it must not do is default to true. Re-justified 2026-09-17 (#1055, review blocker): the comment annotating that declaration quoted a 40,000-entry window count as a chain total — in the codex row, as "40 times — the most of any member", which is true only for a window band and false for the population. All three now name the measurement basis instead of a number; the counts live in findings/review-13031-verdict.md §A, where they can be re-measured, not in a shim comment where they rot. |
