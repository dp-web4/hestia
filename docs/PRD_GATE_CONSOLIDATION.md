...
**Status**: proposed — dp-directed 2026-08-11; peer-reviewed by GPT (NOT-SAME) 2026-08-11 and revised (see §11); fourth pass 2026-08-11 folding kimi's cross-vendor review (notice 1929, see §12) — the step-C pilot seat has read the plan and endorsed it; fifth pass 2026-08-12 folding nomad's codex-seat field data (see §13) — the codex trust-rearm hazard, witness-path parity, held-branch sequencing. Not started.
...
**Harness-specific cutover hazard — codex trust-rearm (nomad, 2026-08-12).** codex persists a per-hook-command trust hash in its config (`[hooks.state."<path>:<event>:<i>:<j>"]` in `config.toml`); *any* change to the hook command line — which is exactly what rewiring onto the shared core is — invalidates it, and in non-interactive runs the hook is then **silently skipped**: no error, no prompt, no log line. Live-measured on Nomad 2026-08-08: a newly added leg was skipped on `codex exec` runs (the sibling already-trusted leg fired; the new one did not) until an operator accepted the interactive trust prompt on the next launch. The kimi pilot is structurally blind to this — kimi has no such trust layer — so step C proves the shared core without ever exercising this failure mode. Cutover for every codex seat therefore carries two mandatory steps: (a) an **interactive re-trust launch per seat** before any unattended run, and (b) the **per-seat liveness probe** of criterion 12. Note the per-leg subtlety: trust is per hook command, so a still-trusted leg can keep firing — and keep attesting a digest — while the changed leg beside it is skipped; criterion 7's self-reported digest alone therefore cannot detect this state.
...
### 7.5 Visibility parity + hook liveness (observed) — the witness path gets the same criteria as the gate path (nomad, 2026-08-12)

Criterion 10 closes the availability asymmetry on the gate path. The witness path has its own measured asymmetry: the codex adapter shipped with **no witness (daemon-ingestion) leg** on the post-tool event, so a fresh codex seat gets **denials-only visibility** — denies arrive via the society gate, allows stop in a local Phase-0 observe log. dp saw exactly this on Nomad ("dashboard only shows denied acts for codex"). Patching per-adapter hook wiring is effort pointed backwards — nomad deliberately declined to re-fight that edit against an architecture being retired — so the requirement moves where the fix is built once: **the shared core/mechanism owns witness ingestion the same way it owns the gate.**

11. **Witness-path parity: allow-side and deny-side chain visibility is structural and equal across harnesses.** A harness wired onto the shared core gets both legs from the shared-mechanism module (§6.E) — witness ingestion is a property of adopting the core, never re-implemented per adapter's hook wiring. Accepted state, observed: after a seat's cutover, allow-side entries from that seat land in the chain (rate comparable to claude's); a seat visible only on the deny side is a red reading. This is criterion 10's sibling — 10 is availability parity on the gate path, 11 is visibility parity on the witness path.

12. **Post-cutover per-seat liveness probe: fired, not wired.** After any change to a seat's hook wiring (cutover, rewire, reinstall, trust-state change), fire a canary tool call on that seat and assert the chain entry landed — allow side AND deny side. A receiver's assumed capability is not evidence about its capability; the probe converts "the hooks are configured" from an attestation into a check. Motivating instance: codex's silent trust-disarm (§6.F), which produces exactly the failure this criterion detects — hooks present on disk, config current, one leg not firing, no signal anywhere.
...
## 13. Revision note — nomad (codex seat) field data incorporated, 2026-08-12

Nomad ran codex under the per-harness adapter through the hackathon weekend (forum: codex-seat notes for gate core, 2026-08-12) and filed three data points; all three adopted:

1. **The codex trust-rearm hazard → §6.F cutover precondition + criterion 12.** codex silently skips a hook whose command line changed, in non-interactive runs, until an interactive launch re-accepts trust — live-measured 2026-08-08, not hypothetical. This is a failure mode the step-C pilot **cannot** surface (kimi has no per-hook trust layer), so "the pilot proved the core" will be true and insufficient when codex rewires. The per-leg subtlety is the sharpest part: a still-trusted leg keeps firing and attesting while the changed leg beside it is skipped, which is why the digest criterion (7) alone cannot catch it and a fired-not-wired probe (12) is load-bearing.
2. **Witness leg is core-owned, not an adapter patch → §7.5 criterion 11.** The shipped codex adapter had no witness leg; fresh installs get denials-only visibility, measured on the dashboard. Nomad explicitly declined to keep patching the per-adapter wiring — correctly, it is the architecture being retired — and asked instead that the shared core own witness ingestion structurally. Availability parity (10) gains its visibility sibling (11).
3. **Held-branch disposition ratified into sequencing.** `nomad/kiro-crush-adapters` and `nomad/antigravity-adapter` stay open and HELD — they are per-harness gate copies of the pre-consolidation shape; merging them now would add two more instances of the thing steps B–E exist to remove. They migrate onto the shared core **after** the codex rewire proves the trust-rearm path (§6.F), codex being the nearest-shaped harness. Carried forward for that migration: antigravity **fails CLOSED natively, the only harness measured here that does** — that property must survive its migration, not be regressed to fail-open-behind-a-fail-closed-shim.
