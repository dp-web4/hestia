...
- **App-contract CI (§6):** drop the silent skip; a job that boots a daemon and runs the `--ignored` tests.
...
| 6 | The app runs identically on laptop, phone, and Jetson. | **Partial.** CI builds all targets; Android APK path is green; iOS unbuilt. The live client-contract test (§6) **does not run in CI and can pass while checking nothing** — this row's "demonstrated by" claims a running check that does not run. |
