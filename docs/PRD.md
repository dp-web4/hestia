...
- **R:** onboarding never exposes raw key material; a non-technical user reaches a working node in minutes — with an identity and a society, not merely a file.
- **Status: this requirement fails as shipped, and the section is ambiguous about its own subject.** In the released artifact `hestia init` creates an **empty vault and nothing else** — `info` reports home, vault path, existence and size; no identity, no society, no LCT, no keypair on disk. The `hestia_first_run` path that v2 named as "the first-run flow" is an **MCP prompt** — agent-facing. So §5.1 currently reads as both a user-facing requirement and a description of an agent-facing prompt, and on the requirement reading it is unmet. **Owed decision (dp/CBP): which one is §5.1?** *(Legion, cold run.)*
...
- **Robust:** survives restarts, network loss, device loss; no data loss of the vault/chain; a denied/failed act leaves state bit-identical. **Live counterexample:** a failed `hestia init` (no tty for the passphrase) exits 1 and leaves an empty `~/.hestia/` behind. Consequence is nil today — re-running succeeds — but it is a real violation of a stated requirement, and the kind that grows teeth. *(Legion, cold run.)*
...
- **Small & fast:** **daemon binary ~12–18 MB — currently 22.97 MB unstripped (`e014552`), i.e. 28% over the top of the target.** Measured, not asserted: `v0.0.3` shipped at **18.24 MB** (target *met*, at the top edge); `main` has grown +25.9% in 69 days (~+2 MB/month); `strip = true` in `[profile.release]` recovers it to 18.44 MB in one line. Sub-second local operations; runs on a phone and a Jetson. The desktop app shell is its own artifact and its own budget; don't read one number as covering both. *(v2 widened this to "low tens of MB," which cannot fail — and did so at the moment the binary crossed the old target, so the hedge could express none of the three facts above. Three reviewers objected on principle; Legion then measured it. This is what a falsifiable target buys and a hedge costs.)*
...
- **Release cadence — newly owned here, and currently the largest gap in the document (§9 crit 1a, §11).** The installable daemon is `v0.0.3` (2026-05-16): **157 `core/` commits stale**, missing every command criterion 1's steps 2–4 need. No desktop app has ever been released. No section of v2 owned this, which is how the PRD came to promise capabilities no user could obtain. Owed: a daemon release off `main`, a desktop app artifact, `strip = true` in `[profile.release]` (§6), and a stated cadence so the gap cannot silently re-open. *(Legion measured it; the fix belongs with whoever owns `release.yml`.)*
...
|---|---|---|
| 1a | The crit-1 path exists **mechanically** in a *shipped artifact*, with no config file and no key handling. | **RUN 2026-07-24 (Legion) — FAILS at step 2.** Fresh `HOME`, no `~/.hestia`, no operator key, release binary only. Step 0 acquire ✅; step 1 `init` ✅ only under a pty, and mints an empty vault and nothing else (§5.1); **step 2 "join a hub" — no such command exists in the shipped binary**; step 3 "add a device" — likewise; step 4 not reached. `v0.0.3` (2026-05-16) predates `hub`/`constellation`/`lct`/`witness`/`profile`/`delegate` by **157 `core/` commits**. Tests the mechanical half only — **never the persona test.** Artifact: `forum/legion-crit1-mechanical-half-run-cold-fails-at-step-0-...-2026-07-24.md`. |
| 1b | A **non-builder** gets through it. | A cold-run by a fleet peer who did not build hestia, on a clean machine, transcript logged. Burns the top blockers on goodwill that renews, before spending an owner's, which doesn't. **Blocked on 1a** — and this is the point of running 1a first: recruiting anyone today would have spent their goodwill discovering that the software they can install cannot do steps 2 or 3. Legion stopped before joining the live fleet hub with a throwaway identity; completing the walk needs a disposable hub or dp's go-ahead. |
...
| 6 | The app runs identically on laptop, phone, and Jetson. | CI builds all targets; the live client-contract test (§6) runs against a real daemon per platform. Honest state, narrower than v2 claimed: **Android is the only app target ever shipped** — all three `app-v0.1.x` releases carry an Android APK and nothing else. Desktop is **built and never released**, which is a different defect from iOS's *unbuilt* and takes a different fix. The client-contract test is not wired to CI (§6), so the one pairing a user could actually try — the 2026-06-13 APK against the 2026-05-16 daemon — has **never been version-matched by anything**. *(Legion, cold run.)* |
...
- **Release lag is the gap between what this document promises and what a person can obtain** (§8, §9 crit 1a). 157 unreleased `core/` commits and no desktop artifact mean criterion 1 is blocked on a *release*, not on recruiting a person — the opposite of what v2 assumed. Worth recording why this went unnoticed: five reviewers read v2 closely and four of us argued about the credential taxonomy, the most interesting thing in the document. The largest actual gap was that you cannot install it. **The tree told us that; we told each other the other thing.** A PRD that names its own artifacts is the correction, and it only worked because someone ran the artifact instead of reading the doc.
