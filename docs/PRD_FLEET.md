...
### 13.5 Subscription-account auth — the honest design

*(Evidence class: fleet-measured + author knowledge as of 2026-08; the external-research pass on this track did not complete — provider-policy verification is an open item, §12.9.)*

The question: can hestia drive models under a member's *subscription* (Claude Pro/Max, ChatGPT Plus) rather than metered API keys? Three postures exist:

1. **Documented/supported:** the vendor's own harness authenticated by the account owner on their own machine — Claude Code's OAuth login (and the Agent SDK riding it), Codex CLI's sign-in-with-ChatGPT. **This fleet already runs on exactly this**, and its properties are measured here: the 2026-08-16/17 weekly-limit outage took every claude-driven track dark for ~2 days while python timers ran on — subscription auth means *shared, rate-limited, resettable* capacity with vendor-side outage semantics that must be modeled (the fleet's own discriminator: a fleet-wide gap in narrative logs with intact data output = account limit, not machine fault).
2. **Gray/fragile:** extracting the CLI's OAuth token and replaying it against the API from third-party code. Observed in the wild; repeatedly contested by providers; ToS-exposed. **Rejected here** — not on enforceability but on the society's own principle: hestia's legitimacy story is consent and legibility; building its model access on a laundered credential contradicts the document's spine.
3. **The design this PRD adopts:** `ModelBackend::Subscription{cli_ref}` = **the vendor's own harness binary, driven headless, as an inference transport** — `claude -p` (as every fleet launcher already does), Codex equivalents likewise. It is posture 1 by construction: the vendor tool, logged in by the account owner, on the owner's machine.

The honest trade-off, stated: a Subscription-backed session is **not fully native** — the vendor harness's own loop sits between hestia and the model, so it is a *hybrid*: hestia composes context, receives output, and gates hestia-dispatched tools natively, but the vendor loop may make its own tool calls governed only by the hooked shim. Assurance profile: A1, same as hooked. **Full A2-by-construction requires `Api` or `Local` backends.** The dashboard derives and displays this per session; nothing is asserted. Vault treatment is uniform: the subscription login state is a credential like any other (Principle 1), and budget accounting reads the vendor's own limit signals rather than pretending token counts it cannot see.
