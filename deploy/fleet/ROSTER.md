...
| 5 | McNugget   | macOS aarch64 (M4)     | aarch64-apple-darwin           | ✅      | ✅      | ✅         | launchd ✓ (`com.web4.hestia.daemon`, pid alive on :7711); daemon v0.0.3; plugin hook (`post_tool_use.sh`) wired in `~/.claude/settings.json`; dashboard HTTP 200. macOS Tauri app (.app + .dmg, v0.1.0) built 2026-05-24. iOS pending full Xcode. **These ✅ certify a hand-written agent, not this repo's template** (mcnugget, 2026-07-25): the live label is `com.web4.hestia.daemon` (written 2026-05-21), the shipped template is `io.hestia.tools` — different label, `ProgramArguments`, and secret handling, so `deploy/templates/io.hestia.tools.plist` has never been run by anyone. Uptime 53 days (boot 2026-06-01) predates the mesh arming, so the launchd ✅ has not survived a reboot either. |
