...
#: ONE predicate, imported, not a second copy. The v1 copy that lived here was the
#: substring rule codex refuted in review 7765 (prose hits: claude 35 of 38 "deaths").
#: `rule="substring"` reproduces it so the published 2026-08-31 numbers stay re-derivable
#: next to the corrected ones; `rule="terminal"` is the rule now.
from dead_wakes_are_not_availability import wake_died  # noqa: E402


def load_wakes(live_only=False, dead_rule="terminal"):
    ...
    for fn in os.listdir(WAKE_DIR):
        ...
        if not member:
            continue
        if live_only and wake_died(os.path.join(WAKE_DIR, fn), rule=dead_rule):
            dropped[member] += 1
            continue
...
def main(argv=None):
    ...
    ap.add_argument("--dead-rule", choices=("terminal", "substring"), default="terminal",
                    help="which dead-wake predicate --live-wakes-only applies. "
                         "'substring' is the refuted v1 (review 7765), kept so the "
                         "published 37.3%% / 7.0%% can be reproduced and their error stated.")
    ap.add_argument("--until", default="",
                    help="ISO-8601 UTC timestamp; chain entries newer than this are "
                         "skipped before the hop budget is spent, so a historical run "
                         "can be re-selected (review 7765, item 4). The head hash of "
                         "the slice actually used is printed either way.")
    args = ap.parse_args(argv)

    wakes = load_wakes(live_only=args.live_wakes_only, dead_rule=args.dead_rule)
    chain = ChainWalker()
    opened, decided = {}, {}
    factors = {}
    ...
    for entry in chain.walk(max_entries=10 ** 9):
        t = entry.get("timestamp")
        if args.until and t and t > args.until:
            skipped += 1
            continue
        if hops >= args.max_hops:
            break
        hops += 1
        head_hash = head_hash or entry.get("hash")
        if t:
    ...
    out = {
        "hops": hops, "skipped_newer_than_until": skipped, "head_hash": head_hash,
        "span_oldest": span_old, "span_newest": span_new,
        "dead_rule": args.dead_rule if args.live_wakes_only else None,
    ...
    print("walk: %d hops, span %s .. %s  (HOP BUDGET, not a date); head %s; "
          "%d entries newer than --until skipped"
          % (hops, span_old, span_new, head_hash, skipped))
