...
The point of this file is deliberately boring: derive activity from Git history and
write ordinary Markdown. Automated GitHub readers can cache or mis-render GitHub's
UI counters; committed text remains inspectable, diffable, and attributable to a
specific snapshot time.
...
Run from a checkout with full history. For each checkout the generator prefers the
remote default branch (origin/HEAD) when available, so running it from a feature
branch does not count the feature branch as default-branch activity.
"""
