...
pub fn is_addressable_member_id(id: &str) -> bool {
    !id.is_empty()
        ...
        && !id.chars().any(|c| {
            c.is_whitespace() || c.is_control() || STRUCTURAL_SEPARATORS.contains(&c)
        })
}
...
mod tests {
    ...
    fn mintable_is_strict_and_addressable_is_structural() {
        ...
        assert!(!is_addressable_member_id(&"a".repeat(MAX_MEMBER_ID_BYTES + 1)));
    }
    ...
    fn empty_segments_are_refused() {
        assert_eq!(parse_address("/thor").unwrap_err(), AddressError::EmptySegment);
        assert_eq!(parse_address("thor/").unwrap_err(), AddressError::EmptySegment);
